...
class BaseTenantDatabase(models.Model):
    ENGINES = (
    ...
    engine = models.CharField(max_length=48, default=DEFAULT_DB['ENGINE'], choices=ENGINES, help_text='Django database engine type')
    ...
    def disconnect(self):
        try:
            db_pre_disconnect(sender=self.__class__, instance=self)
            connections[self.db].close()
            db_post_disconnect(sender=self.__class__, instance=self)
